...
int main() {
    ...
    int position_a_val = (int)'a';
    int counter = 0;
    for (int i = 0; i < matrix.size(); i++) {
        for (int j = 0; j < matrix[i].size(); j++) {
            if (matrix[i][j] == position_a_val) {
                positions_a.push_back(std::pair<int, int>{i, j});
                counter++;
            }
        }
    }
    std::cout << "Number of values with lowest height: "<< positions_a.size() << std::endl;
    ...
    std::vector<Node*> path;
    for (std::pair<int, int> pos : positions_a) {
        
        Node* startNode = graph[pos];
        
        std::pair<int, int> key_to_check{20, 9};
        std::vector<Node*> path_test = BFSearh(startNode, endNode);
        if (!path_test.empty()) {
            if (path_test.size() < path.size() || path.empty()) {
                path = path_test;
