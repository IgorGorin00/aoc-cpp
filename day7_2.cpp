...
//    }
    ...
    int taken_space = file_structure["/"];
    int free_space = total_space - taken_space;
    int need_to_free = needed_space - free_space;
    std::cout << "Taken space: " << taken_space << "\t" 
              << "Free space: " << free_space << "\t" 
              << "Space that needs to be freed: " << need_to_free
              << std::endl;

    std::vector<int> potential_dirs_to_delete;
    for (const auto& pair : file_structure) {
        const int& value = pair.second;
        if (value > need_to_free) {
            potential_dirs_to_delete.push_back(value);
        }
    }

    int result;
    auto size_of_dir_to_delete = std::min_element(potential_dirs_to_delete.begin(), potential_dirs_to_delete.end());
    if (size_of_dir_to_delete != potential_dirs_to_delete.end()) {
        result = *size_of_dir_to_delete;
    } else {
